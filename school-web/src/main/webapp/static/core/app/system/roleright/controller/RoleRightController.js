Ext.define("core.system.roleright.controller.RoleRightController", {
	...
	init: function() {
		...
		this.control({						
			...
			"panel[xtype=roleright.rolgerightgrid] button[ref=gridSetPermission]": {
				beforeclick: function(btn) {
					...
					if (!win) {
						win = Ext.create('core.base.view.BaseFormWin', {
							...
							funCode: detCode,
							items: [{
								xtype: "roleright.selectpmslayout"			
							}],
							listeners:{
								show :function(me){	
																
									//待选的菜单中要过滤已有的菜单								
									var permissionGrid = me.down("basegrid[xtype=roleright.permissiongrid]");
									var selectedPermissionGrid = me.down("basegrid[xtype=roleright.selectedpermissiongrid]");
